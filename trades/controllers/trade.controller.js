...
exports.getOrderDepth = async (req, res) => {
    ...
    try {
        ...
        console.log(`Best Bid Price for ${tradeSymbol}: ${bestBid[0]}, Quantity: ${bestBid[1]}`);
        console.log(`Best Ask Price for ${tradeSymbol}: ${bestAsk[0]}, Quantity: ${bestAsk[1]}`);
        ...
        res.send({
            statusCode: res.statusCode,
            statusMessage: 'success',
            message: 'Successfully retrived bid price',
            bestBid: `Best Bid Price for ${tradeSymbol}: ${bestBid[0]}, Quantity: ${bestBid[1]}`,
            bestAsk: `Best Ask Price for ${tradeSymbol}: ${bestAsk[0]}, Quantity: ${bestAsk[1]}`
        });
        ...
        res.status(500).json({
            statusCode: res.statusCode,
            statusMessage: 'error',
            message: `Error fetching order book depth:', ${error.response ? error.response.data : error.message}`,
        });    
    }
}
